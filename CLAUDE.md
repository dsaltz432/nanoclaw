...
| Fantasy Football | [docs/fantasy-football.md](docs/fantasy-football.md) | Three Sleeper leagues; recommender only (Sleeper's API is read-only). Data layer in `~/Documents/repositories/fantasy-football-agent`, SQLite at `store/ff.db`, entry point `python3 -m ff.cli`. Code mounted read-only — this group reads third-party news text. Dashboard tab at `/fantasy` (today / lineup / moves / trades / rankings / reading / admin), served by `ff.cli api`; the expert-site content layer behind it is described in `fantasy-football-agent/CONTENT-PLAN.md`. Telegram group `Fantasy Football` (`tg:-5468369997`), no trigger word — ask it questions directly. Four host refresh jobs, none of which can message anyone by construction: `com.nanoclaw.ff-news` (15 min, news), `com.nanoclaw.ff-live` (2h, transactions + **rosters**), `com.nanoclaw.ff-daily` (06:40, projections/values), `com.nanoclaw.ff-claims` (15 min, headless `claude -p` claim extraction, no tools). See [docs/host-cronjobs.md](docs/host-cronjobs.md). Note `ff.cli live` does *not* refresh roster contents on its own — `scripts/ff-refresh.sh` adds the `ingest_league_chain` call that does. **No alerting.** The old Fantasy Alerts section was removed 2026-09-16; design fresh when wanted. |
...
| People | [docs/people.md](docs/people.md) · [groups/people/CLAUDE.md](groups/people/CLAUDE.md) | When Daniel last had a real interaction with close friends and family. **Phone and WhatsApp calls load nightly** via host launchd job `com.nanoclaw.people-call-sync` (4:15 AM + 7:00 AM): the phone backs up its call log and its encrypted WhatsApp database to Drive `Agents/Call-Backups` / `Agents/Whatsapp-Backups` at 3:30 AM; the job archives them privately in `~/.config/nanoclaw/people-exports/`, loads new calls for tracked numbers into `data/sessions/people/.claude/contacts.db`, and logs each new call (`~/.local/share/nanoclaw/logs/people-call-sync.log`, also on Admin ▸ Host Tasks). WhatsApp is decrypted **in memory, calls only**, with the key from the macOS Keychain (`whatsapp-backup-key`). Host-only on purpose — backups name everyone; the People container has no Drive token. Plus manual entries over Telegram (`scripts/people-log.py`; the agent never creates a person) and on the dashboard. Metadata only, data from 2025-01-01. People are managed on the dashboard's `/people` page. Weekly `People check` nudge is planned, not built. |
